...
POST /api/auth/login
  ...
  → GET  /api/subscriptions/ai-plans      (history of all AI plans generated by this user)
...
> Result is automatically saved to the `user_ai_plans` table so it can be retrieved later.

---

#### `GET /api/subscriptions/ai-plans`
Auth: any logged-in user

Returns all AI plans generated by the logged-in user. Admin sees all users' records.
...
> Admin response includes `name` and `email` fields per record.
