...
if (!isset($_SESSION['user_role']) || $_SESSION['user_role'] !== 'admin') {
    sendJson(403, false, 'Forbidden: Only admins can create nutritionist accounts');
}
...
$role_name = 'nutritionist'; // Hardcoded for this endpoint
...
try {
    ...
    $db->commit();
    sendJson(201, true, 'Nutritionist created successfully', ['user_id' => $userId]);

} catch (Exception $e) {
    $db->rollback();
    sendJson(500, false, 'Failed to create nutritionist: ' . $e->getMessage());
} finally {
    $db->close();
}
