...
// Verify target user is a nutritionist
...
try {
    ...
    $db->commit();
    sendJson(200, true, 'Nutritionist updated successfully', ['user_id' => $targetUserId]);

} catch (Exception $e) {
    $db->rollback();
    sendJson(500, false, 'Failed to update nutritionist: ' . $e->getMessage());
} finally {
    $db->close();
}
