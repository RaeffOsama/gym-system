...
// Recommended: Check if logged in user is an admin
...
$password = isset($input['password']) ? $input['password'] : '123456'; // Default password if not provided
$role_name = isset($input['role_name']) ? trim($input['role_name']) : 'trainer'; // Default to trainer
...
try {
    ...
    $db->commit();
    sendJson(201, true, 'Specialist user and profile created successfully', ['user_id' => $userId]);

} catch (Exception $e) {
    $db->rollback();
    sendJson(500, false, 'Failed to create specialist: ' . $e->getMessage());
} finally {
    $db->close();
}
