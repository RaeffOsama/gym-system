...
if (!isset($_SESSION['user_role']) || $_SESSION['user_role'] !== 'admin') {
    sendJson(403, false, 'Forbidden: Only admins can delete trainer accounts');
}
...
if ($targetUserId <= 0) {
    sendJson(400, false, 'Valid Trainer User ID is required');
}

$db = getDbConnection();

// Verify user is a trainer before deleting
...
if ($stmt->execute()) {
    $stmt->close();
    $db->close();
    sendJson(200, true, 'Trainer deleted successfully');
} else {
    $stmt->close();
    $db->close();
    sendJson(500, false, 'Failed to delete trainer');
}
